...
Keep the product logic stable: do not add new gates or make legal-compliance certification claims. Add only one extra view inside the existing gates: data and privacy handling, used when files, user content, model calls, logs, or demo production gaps are involved.
...
- Keep the first answer simple for both audiences: allowed scope, blocked scope, top risks, and next proof needed.
...
- Data and privacy handling: what user data, files, AI context, model output, or logs exist, and whether their path is known.
...
2. Run the five-gate review.
   ...
   - When a relevant gate touches user data, uploaded files, model context, model output, or logs, add the data and privacy handling view: what data exists, where it goes, whether logs keep sensitive content, who can access it, how long it stays, and whether there is proof.
   - Treat this as evidence inside the existing gates, especially image/file upload risk, AI prompt/model-call exposure, and demo-to-public-product gap. Do not create a separate legal compliance gate.
...
4. Decide the launch status.
   ...
   - If using Delivery Acceptance mode, turn the same evidence into a plain-language delivery decision:
     ...
     - Add one short data and privacy handling feedback block when relevant.
...
Privacy and data handling rules may inform evidence expectations, but the skill must not claim that a product is legally compliant. It can only say what data/privacy evidence is visible or missing and how that affects the scoped launch decision.
