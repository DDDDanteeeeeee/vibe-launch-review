...
- 涉及的数据或文件：客户 `.ai` 文件、本地任务副本、命中规则的文本片段、AI 输入、AI 输出、点数扣费记录和云端任务日志。
- 去哪里：客户 `.ai` 文件在客户端本地处理；命中规则的文本片段会发送到云端 `/api/translate/batch`，再由云端发送给 DeepSeek；客户端不内置 DeepSeek key。
- 日志/访问/保留/删除证据：已有本地任务数据清理入口，但本轮没有复查生产服务器 env 权限、日志脱敏、访问权限、保留时间和删除策略。
- 对受控试点的影响：不直接阻塞受控试点，因为客户数量、点数发放和任务跟进都由运营方控制。
- 对公开发布的影响：阻塞 public self-serve launch / large-scale public release；公开放量前需要补齐日志脱敏、权限、保留/删除、预算、告警和熔断证据。
