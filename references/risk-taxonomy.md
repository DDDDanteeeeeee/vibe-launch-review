...
- No evidence of who can access uploaded files, how long they remain, or whether raw file contents are logged.
...
- User files can contain personal, customer, or business-sensitive data; public launch needs evidence that access, retention, logs, and deletion are controlled.
...
- Recommend data-handling evidence when files may contain customer, personal, or business-sensitive content.
...
- Full user files, personal data, customer documents, or unrelated context sent to model calls without visible minimization.
- Raw prompts, model inputs, model outputs, or provider payloads stored in logs without visible redaction or retention boundary.
...
- Model calls can leak more data than the task requires if the product sends whole files or raw user content without minimization.
...
- Recommend evidence that only necessary context is sent to the model and that logs do not keep raw sensitive prompts or outputs.
...
- Demo logs that keep full request bodies, uploaded file content, user prompts, model outputs, or secrets without production retention and access rules.
- No visible cleanup or deletion path when the product stores real user data.
...
- Demo logging and storage assumptions can expose real user data once the product leaves a trusted test environment.
...
- Recommend re-review with production log redaction, data retention, and deletion evidence when user data is stored or processed.
